...
{
	const ssl_file_format_t ssl_file_format = file_format == crypto_file_format_t::asn1 ? ssl_file_format_t::asn1 : ssl_file_format_t::pem;

	native_handle_->use_certificate_file(path_to_certificate, ssl_file_format);
}
...
void boost_ssl_context_t::use_private_key(const std::string& path_to_key, const crypto_file_format_t file_format)
{
	...
	native_handle_->use_private_key_file(path_to_key, ssl_file_format);
}

void boost_ssl_context_t::disable_certificate_verification()
{
	native_handle_->set_verify_callback(
		[](const bool is_pre_verified, boost::asio::ssl::verify_context& verify_context) -> bool
		{
			return true;
		}
	);
